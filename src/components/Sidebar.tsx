...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className={`w-64 h-screen bg-[var(--color-panel)] border-r border-[var(--color-border)] flex flex-col fixed print:hidden z-50 transition-transform duration-300 ${isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 rtl:right-0 rtl:left-auto rtl:border-r-0 rtl:border-l rtl:md:translate-x-0 rtl:${isMobileSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-16 flex items-center justify-center border-b border-[var(--color-border)] p-2 relative">
          ...
          <button 
            className="absolute right-4 text-gray-400 hover:text-white md:hidden"
            onClick={closeMobileSidebar}
          >
            <X size={20} />
          </button>
