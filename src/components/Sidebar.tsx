    ...
    <>
      ...
      <aside className={`w-full md:w-72 h-screen bg-[var(--color-panel)] border-r border-[var(--color-border)] flex flex-col fixed print:hidden z-50 transition-transform duration-300 ${isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 rtl:right-0 rtl:left-auto rtl:border-r-0 rtl:border-l rtl:md:translate-x-0 rtl:${isMobileSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <nav className="flex-1 py-6 px-4 space-y-2 overflow-y-auto custom-scrollbar">
          ...
          {can('services', 'view') && <NavItem href="/packages" icon={<Layers size={20} />} label="Groom Packages" active={pathname === '/packages'} onClick={closeMobileSidebar} locked={!hasFeature('packages') || isPlanExpired()} isExpired={isPlanExpired()} />}
          {can('services', 'view') && <NavItem href="/deals" icon={<Package size={20} />} label="Deals" active={pathname === '/deals'} onClick={closeMobileSidebar} locked={!hasFeature('deals') || isPlanExpired()} isExpired={isPlanExpired()} />}
          {can('services', 'view') && <NavItem href="/promotions" icon={<DollarSign size={20} />} label="Promo Codes" active={pathname === '/promotions'} onClick={closeMobileSidebar} locked={!hasFeature('promo_codes') || isPlanExpired()} isExpired={isPlanExpired()} />}
