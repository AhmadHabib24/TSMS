...
export default function ReportsPage() {
  ...
  const [dateFilter, setDateFilter] = useState('all'); // today, yesterday, week, month, custom
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [paymentMethod, setPaymentMethod] = useState('');

    // Calculate dates when dateFilter changes
  useEffect(() => {
  ...
  // Unified Fetch logic when tab, dates, or payment method changes
  useEffect(() => {
    // If it's udhar, it doesn't depend on dates
    if (activeTab === 'udhar') {
      setIsLoading(true);
    ...
    // For all other tabs, wait until dates are calculated
    if (!startDate || !endDate) return;

    if (activeTab === 'sales') fetchSalesReport();
    else if (activeTab === 'pnl') fetchPnlReport();
    else if (activeTab === 'inventory') fetchInventoryReport();
    else if (activeTab === 'discounts') fetchDiscountReport();
      ...
      api.get(`${url}?${params.toString()}`).then(res => {
        setBills(res.data);
        setIsLoading(false);
      }).catch(() => setIsLoading(false));
      ...
      api.get(`${staffQs}?${params.toString()}`).then(res => {
        ...
        <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-4 sm:p-6 flex flex-col xl:flex-row gap-4 sm:gap-6 justify-between items-start xl:items-center animate-in fade-in duration-500">
          <div className="flex flex-wrap gap-2 w-full xl:w-auto">
            {['today', 'yesterday', 'week', 'month', 'all'].map(f => (
              <button key={f} onClick={() => { setIsLoading(true); setDateFilter(f); }} className={`flex-1 sm:flex-none px-2 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-bold capitalize transition-colors ${dateFilter === f ? 'bg-white text-black' : 'bg-black/30 text-gray-400 border border-[var(--color-border)] hover:border-white'}`}>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full xl:w-auto">
            <div className="bg-black/30 border border-[var(--color-border)] rounded-lg px-4 py-2 flex items-center gap-2 w-full sm:w-auto">
              ...
              <input type="date" value={startDate} onChange={e => { setIsLoading(true); setStartDate(e.target.value); setDateFilter('custom'); }} className="bg-transparent w-full text-sm text-[var(--color-foreground)] outline-none" />
            </div>
            <span className="text-gray-500 font-bold text-center">TO</span>
            <div className="bg-black/30 border border-[var(--color-border)] rounded-lg px-4 py-2 flex items-center gap-2 w-full sm:w-auto">
              <Calendar size={16} className="text-[var(--color-gold)] shrink-0" />
              <input type="date" value={endDate} onChange={e => { setIsLoading(true); setEndDate(e.target.value); setDateFilter('custom'); }} className="bg-transparent w-full text-sm text-[var(--color-foreground)] outline-none" />
            </div>
            ...
            <div className="bg-black/30 border border-[var(--color-border)] rounded-lg px-4 py-2 flex items-center gap-2 w-full sm:w-auto ml-0 xl:ml-4">
              ...
              <select value={paymentMethod} onChange={e => { setIsLoading(true); setPaymentMethod(e.target.value); }} className="bg-transparent w-full text-sm text-[var(--color-foreground)] outline-none">
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-[var(--color-panel)] border border-green-500/30 rounded-xl p-6 shadow-[0_0_15px_rgba(16,185,129,0.05)] relative overflow-hidden group">
              ...
              <div className="text-3xl font-black text-[var(--color-foreground)] mb-2">
                {isLoading ? <div className="h-9 w-32 animate-pulse bg-white/10 rounded"></div> : `₨ ${pnlReport?.total_income || 0}`}
              </div>
              <div className="text-xs text-gray-400">
                {isLoading ? <div className="h-8 w-40 animate-pulse bg-white/10 rounded mt-1"></div> : <>Sales: ₨ {pnlReport?.total_sales || 0} <br /> Manual: ₨ {pnlReport?.total_manual_income || 0}</>}
              </div>
            </div>
            ...
            <div className="bg-[var(--color-panel)] border border-red-500/30 rounded-xl p-6 shadow-[0_0_15px_rgba(239,68,68,0.05)] relative overflow-hidden group">
              ...
              <div className="text-3xl font-black text-[var(--color-foreground)] mb-2">
                {isLoading ? <div className="h-9 w-32 animate-pulse bg-white/10 rounded"></div> : `₨ ${pnlReport?.total_expenses || 0}`}
              </div>
              <div className="text-xs text-gray-400">
                {isLoading ? <div className="h-4 w-32 animate-pulse bg-white/10 rounded mt-1"></div> : `Across ${pnlReport?.expenses_by_category?.length || 0} categories`}
              </div>
            </div>
            ...
            <div className={`bg-gradient-to-br ${(pnlReport?.net_profit || 0) >= 0 ? 'from-green-500/20 to-teal-500/5 border-green-500/50 shadow-[0_0_30px_rgba(16,185,129,0.1)]' : 'from-red-500/20 to-orange-500/5 border-red-500/50 shadow-[0_0_30px_rgba(239,68,68,0.1)]'} border rounded-xl p-6`}>
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <div className="text-4xl font-black text-[var(--color-foreground)]">
                    {isLoading ? <div className="h-10 w-36 animate-pulse bg-white/10 rounded"></div> : `₨ ${pnlReport?.net_profit || 0}`}
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-6 h-[400px]">
              ...
              <div className="h-[280px]">
                {isLoading ? <div className="w-full h-full animate-pulse bg-white/5 rounded-lg"></div> : <Bar data={pnlBreakdownData} options={commonOptions} />}
              </div>
            </div>
            ...
            <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-6 h-[400px]">
              ...
              {isLoading ? (
                <div className="w-full h-[280px] animate-pulse bg-white/5 rounded-full aspect-square mx-auto"></div>
              ...
              <table className="w-full text-left whitespace-nowrap">
                ...
                <tbody className="divide-y divide-[var(--color-border)]">
                  {isLoading ? (
                    [1,2,3].map(i => (
                      <tr key={i}>
                        <td className="py-3"><div className="h-5 w-24 animate-pulse bg-white/10 rounded"></div></td>
                        <td className="py-3 flex justify-end"><div className="h-5 w-20 animate-pulse bg-white/10 rounded"></div></td>
                      </tr>
                    ))
                  ...
                  {!isLoading && (!pnlReport?.payment_methods || pnlReport.payment_methods.length === 0) && (
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-1 bg-gradient-to-br from-[var(--color-panel)] to-black border border-[var(--color-gold)] rounded-xl p-8 flex flex-col justify-center shadow-[0_0_30px_rgba(212,175,55,0.05)]">
              ...
              <div className="text-5xl font-black text-[var(--color-gold)]">
                {isLoading ? <div className="h-12 w-48 animate-pulse bg-[var(--color-gold)]/20 rounded"></div> : `₨ ${salesReport?.total_revenue || 0}`}
              </div>
            ...
            <div className="md:col-span-2 bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-6 h-[300px]">
              ...
              <div className="h-[220px]">
                {isLoading ? <div className="w-full h-full animate-pulse bg-white/5 rounded-lg"></div> : <Line data={dailyChartData} options={commonOptions} />}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-6 h-[350px]">
              ...
              <div className="h-[270px]">
                {isLoading ? <div className="w-full h-full animate-pulse bg-white/5 rounded-lg"></div> : <Bar data={monthlyChartData} options={commonOptions} />}
              </div>
            </div>
            ...
            <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-6 h-[350px]">
              ...
              <div className="h-[270px]">
                {isLoading ? <div className="w-full h-full animate-pulse bg-white/5 rounded-lg"></div> : <Line data={yearlyChartData} options={commonOptions} />}
              </div>
            </div>
          </div>
            ...
            <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-6 overflow-x-auto">
              ...
              <table className="w-full text-left whitespace-nowrap">
                ...
                <tbody className="divide-y divide-[var(--color-border)]">
                  ...
                  {!isLoading && (!salesReport?.payment_methods || salesReport.payment_methods.length === 0) && (
                    <tr><td colSpan={2} className="py-6 text-center text-gray-500">No data available</td></tr>
                  )}
                </tbody>
              </table>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-6 flex flex-col items-center justify-center text-center">
              ...
              <div className="text-4xl font-black text-[var(--color-foreground)]">
                {isLoading ? <div className="h-10 w-24 animate-pulse bg-white/10 rounded"></div> : inventoryReport?.total_unique_items || 0}
              </div>
            ...
            <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-6 flex flex-col items-center justify-center text-center">
              ...
              <div className="text-4xl font-black text-[var(--color-foreground)]">
                {isLoading ? <div className="h-10 w-24 animate-pulse bg-white/10 rounded"></div> : inventoryReport?.total_stock || 0}
              </div>
            ...
            <div className={`bg-[var(--color-panel)] rounded-xl p-6 flex flex-col items-center justify-center text-center ${inventoryReport?.low_stock_count > 0 ? 'border border-red-500/50 shadow-[0_0_20px_rgba(239,68,68,0.1)]' : 'border border-[var(--color-border)]'}`}>
              ...
              <div className={`text-4xl font-black ${inventoryReport?.low_stock_count > 0 ? 'text-red-500' : 'text-[var(--color-foreground)]'}`}>
                {isLoading ? <div className="h-10 w-24 animate-pulse bg-white/10 rounded"></div> : inventoryReport?.low_stock_count || 0}
              </div>
                ...
                <div className="space-y-4">
                  {[1,2,3].map(i => (
                    <div key={i} className="flex justify-between items-center border-b border-[var(--color-border)] pb-3 last:border-0 last:pb-0">
                      <div className="h-5 w-32 animate-pulse bg-white/10 rounded"></div>
                      <div className="h-6 w-20 animate-pulse bg-white/10 rounded-full"></div>
                    </div>
                  ))}
                </div>
              ...
              {isLoading ? (
                <div className="space-y-4">
                  {[1,2,3].map(i => (
                    <div key={i} className="flex justify-between items-center border-b border-[var(--color-border)] pb-3 last:border-0 last:pb-0">
                      <div>
                        <div className="h-5 w-32 animate-pulse bg-white/10 rounded mb-1"></div>
                        <div className="h-3 w-20 animate-pulse bg-white/10 rounded"></div>
                      </div>
                      <div className="h-6 w-16 animate-pulse bg-white/10 rounded"></div>
                    </div>
                  ))}
                </div>
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <div className="text-2xl font-black text-[var(--color-foreground)]">
                    {isLoading ? <div className="h-8 w-32 animate-pulse bg-white/10 rounded"></div> : staffPerformance[0]?.name || '--'}
                  </div>
                </div>
                <Trophy size={32} className="text-yellow-500" />
              </div>
              <div className="text-sm text-gray-300">
                {isLoading ? <div className="h-4 w-40 animate-pulse bg-white/10 rounded mt-1"></div> : <>Generated <span className="text-yellow-500 font-bold">₨ {staffPerformance[0]?.revenue || 0}</span> in revenue</>}
              </div>
            </div>
            ...
            <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-6">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <div className="text-2xl font-black text-[var(--color-foreground)]">
                    {isLoading ? <div className="h-8 w-32 animate-pulse bg-white/10 rounded"></div> : `₨ ${staffPerformance.reduce((acc, curr: any) => acc + curr.revenue, 0)}`}
                  </div>
                </div>
                <DollarSign size={32} className="text-[var(--color-gold)]" />
              </div>
              <div className="text-sm text-gray-400">
                {isLoading ? <div className="h-4 w-32 animate-pulse bg-white/10 rounded mt-1"></div> : `Across ${staffPerformance.reduce((acc, curr: any) => acc + curr.total_bills, 0)} total bills`}
              </div>
            </div>
            ...
            <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-6">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <div className="text-2xl font-black text-[var(--color-foreground)]">
                    {isLoading ? <div className="h-8 w-24 animate-pulse bg-white/10 rounded"></div> : staffPerformance.reduce((acc, curr: any) => acc + curr.customers_served, 0)}
                  </div>
                </div>
              ...
              <table className="w-full text-left whitespace-nowrap">
                ...
                <tbody className="divide-y divide-[var(--color-border)]">
                  {isLoading ? (
                    [1,2,3].map(i => (
                      <tr key={i}>
                        <td className="py-4 px-6"><div className="h-8 w-8 animate-pulse bg-white/10 rounded-full"></div></td>
                        <td className="py-4 px-6 flex items-center gap-3"><div className="h-10 w-10 animate-pulse bg-white/10 rounded-full"></div><div><div className="h-4 w-24 animate-pulse bg-white/10 rounded mb-1"></div><div className="h-3 w-16 animate-pulse bg-white/10 rounded"></div></div></td>
                        <td className="py-4 px-6"><div className="h-5 w-20 animate-pulse bg-white/10 rounded"></div></td>
                        <td className="py-4 px-6"><div className="h-6 w-12 animate-pulse bg-white/10 rounded-full"></div></td>
                        <td className="py-4 px-6"><div className="h-5 w-16 animate-pulse bg-white/10 rounded"></div></td>
                        <td className="py-4 px-6"><div className="h-5 w-24 animate-pulse bg-white/10 rounded"></div></td>
                      </tr>
                    ))
          ...
          <table className="w-full text-left whitespace-nowrap [&_td]:pr-4 [&_th]:pr-4">
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {isLoading ? (
                [1,2,3].map(i => (
                  <tr key={i}>
                    <td className="py-4"><div className="h-5 w-20 animate-pulse bg-white/10 rounded"></div></td>
                    <td className="py-4"><div className="h-5 w-32 animate-pulse bg-white/10 rounded"></div></td>
                    <td className="py-4"><div className="h-5 w-24 animate-pulse bg-white/10 rounded"></div></td>
                    <td className="py-4"><div className="h-5 w-16 animate-pulse bg-white/10 rounded"></div></td>
                    <td className="py-4"><div className="h-5 w-16 animate-pulse bg-white/10 rounded"></div></td>
                    <td className="py-4"><div className="h-5 w-32 animate-pulse bg-white/10 rounded"></div></td>
                    <td className="py-4"><div className="h-5 w-24 animate-pulse bg-white/10 rounded"></div></td>
                  </tr>
              ...
              {!isLoading && discountReport.length === 0 && <tr><td colSpan={7} className="py-12 text-center text-gray-500">No discounts given in this period.</td></tr>}
            </tbody>
          </table>
        </div>
      ) : activeTab === 'udhar' ? (
        <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-4 sm:p-6 animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-x-auto">
          ...
          <table className="w-full text-left whitespace-nowrap [&_td]:pr-4 [&_th]:pr-4">
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {isLoading ? (
                [1,2,3].map(i => (
                  <tr key={i}>
                    <td className="py-4">
                      <div className="h-5 w-32 animate-pulse bg-white/10 rounded mb-1"></div>
                      <div className="h-3 w-24 animate-pulse bg-white/10 rounded"></div>
                    </td>
                    <td className="py-4"><div className="h-5 w-24 animate-pulse bg-white/10 rounded"></div></td>
                    <td className="py-4"><div className="h-5 w-24 animate-pulse bg-white/10 rounded"></div></td>
                    <td className="py-4"><div className="h-6 w-24 animate-pulse bg-white/10 rounded"></div></td>
                    <td className="py-4"><div className="h-6 w-32 animate-pulse bg-white/10 rounded-full"></div></td>
                  </tr>
                ))
              ...
              {!isLoading && udharReport.length === 0 && <tr><td colSpan={5} className="py-12 text-center text-gray-500">No udhar records found.</td></tr>}
            </tbody>
          </table>
        </div>
      ) : (
        <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-4 sm:p-6 animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-x-auto">
          <table className="w-full text-left whitespace-nowrap [&_td]:pr-4 [&_th]:pr-4">
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {isLoading ? (
                [1,2,3,4].map(i => (
                  <tr key={i}>
                    ...
                    <td className="py-4"><div className="h-5 w-24 animate-pulse bg-white/10 rounded"></div></td>
                    <td className="py-4"><div className="h-5 w-16 animate-pulse bg-white/10 rounded"></div></td>
                    <td className="py-4"><div className="h-6 w-20 animate-pulse bg-white/10 rounded"></div></td>
                    <td className="py-4"><div className="h-8 w-24 animate-pulse bg-[var(--color-gold)]/20 rounded"></div></td>
                  </tr>
                ))
              ) : bills.map((b: any) => (
              ...
              {!isLoading && bills.length === 0 && <tr><td colSpan={7} className="py-12 text-center text-gray-500">No bills generated yet.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
