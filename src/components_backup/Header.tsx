...
export default function Header() {
  ...
  return (
    <header className="h-16 bg-[var(--color-background)] border-b border-[var(--color-border)] flex items-center justify-between px-6 sticky top-0 z-10 print:hidden">
      <div>
        <h2 className="text-xl font-semibold capitalize">Welcome back, {user?.name || 'User'}</h2>
        <p className="text-sm text-gray-400">Role: <span className="text-[var(--color-gold)]">{user?.role?.name || 'Unknown'}</span></p>
      </div>
