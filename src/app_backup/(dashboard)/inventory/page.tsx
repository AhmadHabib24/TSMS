  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[var(--color-border)] pb-4">
        <h1 className="text-3xl font-bold flex items-center gap-3"><Package className="text-[var(--color-gold)]" size={32} /> Inventory Management</h1>
        
        <div className="flex bg-[var(--color-background)] border border-[var(--color-border)] p-1 rounded-lg">
          <button onClick={() => setActiveTab('stock')} className={`px-4 md:px-6 py-2 rounded-md font-bold transition-all flex items-center gap-2 ${activeTab === 'stock' ? 'bg-[var(--color-gold)] text-black shadow-md' : 'text-gray-400 hover:text-white'}`}>
            <Package size={18} /> Stock & Alerts
          </button>
          <button onClick={() => setActiveTab('history')} className={`px-4 md:px-6 py-2 rounded-md font-bold transition-all flex items-center gap-2 ${activeTab === 'history' ? 'bg-[var(--color-gold)] text-black shadow-md' : 'text-gray-400 hover:text-white'}`}>
            <History size={18} /> History & Reports
          </button>
        </div>
      </div>
      ...
      {activeTab === 'stock' && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex justify-end">
             {can('inventory', 'add') && (
              <button onClick={() => openModal()} className="bg-[var(--color-gold)] text-black px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-[var(--color-gold-hover)] transition-colors"><Plus size={20} /> Add New Product</button>
            )}
          </div>
          <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl p-6 overflow-x-auto">
            <table className="w-full text-left">
              <thead><tr className="border-b border-[var(--color-border)] text-gray-400"><th>Image</th><th>Item Name</th><th>Stock Level</th><th>Threshold</th><th>Status</th><th>Actions</th></tr></thead>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-[var(--color-border)] bg-[var(--color-background)]">
          ...
          <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-xl w-full max-w-sm overflow-hidden">
             <div className="flex justify-between items-center p-6 border-b border-[var(--color-border)] bg-[var(--color-background)]">
              <h2 className="text-xl font-bold flex flex-col">
                ...
                <span className="text-[var(--color-gold)] text-sm">{adjustingItem.name}</span>
              </h2>
